...
//Синтаксис с помощью конструктора (не популярно) 
...
//Классы:
// \d - ищем цифры    \D - ищем НЕ числа
// \w - ищем слова    \W - ищем НЕ буквы
// \s - ищем пробелы  \S - ищем НЕ пробелы  

// Данные методы относятся к строкам!

//метод search - пытается что-то найти(самый простой метод), flag - g  не сработает
/* const ans = prompt('Введите ваше имя');
const reg = /n/;
console.log(ans.search(reg)); */

//метод match - получем массив в виде результата
/* const ans = prompt('Введите ваше имя');
const reg = /n/ig;
console.log(ans.match(reg));  */

//метод replace - получем массив в виде результата
/* const pass = prompt('Введите ваше пароль');
console.log(pass.replace(/./g, "*"));  */
//   /./g, "*" - берем все элементы нашей строки и зменяем на *
//   /\./g, "*" - если мы хотим найти и заменить точку, нам ее нужно экранировать /\./


//Метод,который относится к регулярным выражениям
// test - особенность метода, он вернет нам true / false